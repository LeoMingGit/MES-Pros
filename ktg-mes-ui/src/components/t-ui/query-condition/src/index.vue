  ...
  >
    ...
    >
      ...
      <template v-if="opt.labelRender" #label>
        <render-comp :create-element-func="opt.labelRender" />
      </template>
      <!-- 自定义输入框插槽 -->
    ...
    >
      <el-button class="btn_check" :loading="loading" v-bind="queryAttrs" @click="checkHandle">查询</el-button>
      ...
      <el-button v-if="originCellLength > colLength&&isShowOpen" type="text" @click="openCilck">
        {{ open ==true ? '收起' : '展开' }}
        <i :class="open ==true ? 'el-icon-arrow-up' : 'el-icon-arrow-down'"></i>
      </el-button>
    </el-form-item>
  </el-form>
</template>
  ...
  computed: {
    ...
    cOpts() {
      ...
      return Object.keys(opts).reduce((acc, field) => {
        ...
        if (isShowOpen) {
          ...
          renderSpan += opt.span !== undefined && opt.span !== null ? opt.span : 1;
    ...
    gridAreas() { // grid布局按钮位置
      ...
      for (let fieldIndex = 0; fieldIndex < fields.length; fieldIndex++) {
        ...
        const span = Math.min(opt.span !== undefined && opt.span !== null ? opt.span : 1, 4);
    ...
    cellLength() { // 占用单元格长度
      let length = 0
      Object.keys(this.opts).forEach(key => {
        const span = this.opts[key].span > 4 ? 4 : this.opts[key].span || 1
        length += span
      })
      return length
    },
    originCellLength() {
      ...
      let length = 0
      Object.keys(this.opts).forEach(key => {
        const span = this.opts[key].span || 1
