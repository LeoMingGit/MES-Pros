<template>
  <el-form-item :label="labelText">
    <el-select
      ...
      :filterable="useVirtual==null?false:filterable"
    ...
    >
      <template #empty>
        <div class="t-table-select__table" :style="{ width: `${tableWidth}px` }">
          <div v-if="isShowQuery" class="table_query_condition">
          ...
          <el-table
            ...
            border
            :row-key="getRowKey"
            :max-height="useVirtual==null?maxHeight||540:maxHeight"
          ...
          >
            ...
            <template v-for="(item, index) in columns">
              <el-table-column
                ...
                :fixed="item.fixed"
                :show-overflow-tooltip="useVirtual==null?true:item.noShowTip?false:true"
...
export default {
  ...
  data() {
    return {
      ...
      tableData: this.table == null ? null : this.table.data, // table数据
  ...
  methods: {
    ...
    selectKeyup(e) {
      if (!this.multiple) {
        ...
        const refsElTable = this.$refs['el-table']
        switch (e.keyCode) {
          case 40: // 下键
