<template>
  <div>
    <el-divider content-position="left">門禁群組明細</el-divider>

    <el-form size="mini" inline>
      <el-form-item>
        <el-button icon="el-icon-search" @click="showSearch" :disabled="!addDetailForm.group_id">查詢</el-button>
        <el-button type="success" v-permission="permission.add" icon="el-icon-plus" @click="insertEvent" :disabled="!addDetailForm.group_id">新增</el-button>
   ...
   <el-table ref="table" :data="tableData" style="width: 100%;" >
      ...
      <el-table-column prop="mac_no" label="門禁設備編號"/>
      <el-table-column prop="mac_name" label="門禁設備名稱"/>
      <el-table-column prop="fac_no_ext" label="廠別"/>
      <el-table-column prop="mgr_no_ext" label=" 負責人工號"/>
      ...
      <!-- <el-table-column prop="spl_flag_ext" label="是否特殊權限" /> -->
...
import { QueryGroupDoorDetailList,DeleteAccessGroupDetail} from '@/api/hikvision/accessGroup'
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      this.doQueryGroupDoorDetailList();
    },
    removeEvent(row) {
      this.$confirm('此操作將移除該項，是否繼續？', '提示', {
        ...
        DeleteAccessGroupDetail(deleteData).then(res => {  
          if (res.Status == 1) {
    ...
    doQueryGroupDoorDetailList(){

      this.filterSearchForm.pageNum=this.tablePage.page;
      this.filterSearchForm.pageSize=this.tablePage.size;  
      this.filterSearchForm.groupId =this.addDetailForm.group_id;
      
      QueryGroupDoorDetailList(this.filterSearchForm).then((res) => {
           if (res.Status == 1) {
    ...
    search(searchForm) { 
      if(searchForm){
        this.filterSearchForm.doorName=searchForm.doorName;
    ...
    editRow(row) {
        ...
        this.editDetailForm.spl_flag=row.special_flag;
        this.editDetailForm.group_name=this.addDetailForm.group_name;
