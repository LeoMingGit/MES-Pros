<template>
  <el-dialog :title="isAddFlag ? '新增角色' : '修改角色'" :visible.sync="dialogVisible" width="800px"
    :before-close="closeDialog" :close-on-click-modal="false">
    <el-form ref="Form" :inline="true" :model="form" :rules="rules" size="small" label-width="66px">
      <el-form-item label="角色編號" label-width="100px" prop="role_id" v-if="!isAddFlag">
      ...
      <el-form-item label="角色廠別" prop="fac_no" label-width="100px">
        <el-select v-model="form.fac_no"   filterable remote   placeholder="請输入角色廠別名稱" @change="$forceUpdate()"
      ...
      <el-form-item label="角色負責人" label-width="100px" prop="mgr_no">
        <el-select v-model="form.mgr_no"   filterable remote   placeholder="請输入角色負責人姓名"   @change="$forceUpdate()"
...
import { AddPersonRole, UpdatePersonRole, queryFactoryDict,queryPrincipalList} from '@/api/hikvision/personGroup'
export default {
  name: "add",
  props: {
    form: {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        fac_no: [
          { required: true, message: '請輸入角色廠別', trigger: 'blur' }
        ],
        mgr_no: [
          { required: true, message: '請輸入負責人工號', trigger: 'blur' }
        ],
        layer: [
          { required: true, message: '請輸入層級', trigger: 'blur' }
        ],
        flag: [
  ...
  methods: {
    ...
    submit() {
      ...
      const api = this.isAddFlag ? AddPersonRole : UpdatePersonRole;
